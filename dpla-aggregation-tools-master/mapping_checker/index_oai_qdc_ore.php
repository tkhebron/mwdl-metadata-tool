...
<html>
    <head>
        <title>OAI Aggregation Tools &raquo; QDC Mapping Checker- Oregon</title>
    ...
    <body id="mappingchecker">
        ...
        <h1>Mapping Checker - Qualified Dublin Core (Oregon)</h1>
        <p>Check incoming oai_qdc feeds for correct qualified Dublin Core mappings. Bright yellow fields are required fields for Mountain West Digital Library. Light yellow fields are recommended fields for MWDL.</p>
        <div id="formwrap">
          ...
          <?php
                    ...
                    <input type="submit"/>
                    <a class="clearbutton" href="index_oai_qdc.php">&times;</a>
        ...
        if ((isset($_GET['base'])) && (isset($_GET['set']))) {
            ...
            if (($base !== '') && ($set !== '')) {
                $thisfeed = $base . "?verb=ListRecords&set=" . $setspec . "&metadataPrefix=oai_qdc";
            } }
        }
        ...
        if (isset($thisfeed)) {
            ...
            if ($thisfeed != '') {
                ...
                if (@$xml->load($thisfeed) === false) {
                    ...
                    $xsl = new DOMDocument;
                    $xsl->load('xsl/extract_qdc_ore.xsl');
